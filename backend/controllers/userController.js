...
exports.login = async function (req, res, next) {
  ...
  if (!email || !password)
    return res.status(400).json({ status: "Enter email && password" });
  const user = await User.findOne({ email: email }).select("+password");
  if (!user || !(await user.correctPassword(password, user.password)))
    return res.status(401).json({ status: "enter valid email and password" });
  // console.log(user.id);
  const id = user.id;
...
exports.forgot = async (req, res, next) => {
  ...
  if (user == null)
    return res.status(400).json({ status: "please enter valid email address" });
